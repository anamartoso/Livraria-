$(document).ready(function(){
    ...
    // Função para exibir todos os piratas
    $.ajax(url)
        .done(function(xml){
            $(xml).find("livro").each(function(){
                $("#cards").append('<div class="card"><a href="individual.html?id='+$(this).find("id").text()+'"><p class="título">Procurado</p> <img class="foto"src="'+ $(this).find("id").text() +'.jpg"><p class="vivo">Vivo ou Morto</p> <p class="nome">'+ $(this).find("nome").text() +'</p> <p>'+ $(this).find("alcunha").text()+'<\p> <p class="rec"> <img class="berrys" src="Berrys.png">'+ $(this).find("recompensa").text()+' </p><p> Status:'+ $(this).find("status").text()+'</a> </p><\div>');
            });
        })
        .fail(function(){
    ...
    // Função para exibir detalhes de um pirata individual
